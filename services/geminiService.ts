...
export const optimizeNodePrompt = async (nodeInfo: AINodeInfo): Promise<OptimizationResult> => {
  const modelId = "gemini-2.5-flash"; // Using Flash for speed and high context window
  ...
  const prompt = `
    ...
    **Current System Prompt:**
    ${nodeInfo.currentSystemPrompt || "N/A"}

    **Current User Prompt:**
    ${nodeInfo.currentUserPrompt || "N/A"}
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
            type: Type.OBJECT,
            properties: {
                systemPrompt: { type: Type.STRING, description: "The optimized system instruction for the AI agent." },
                userPrompt: { type: Type.STRING, description: "The optimized user message/prompt." },
                reasoning: { type: Type.STRING, description: "Explanation of the improvements made." }
            },
