    ...
    <div className="animate-fade-in">
      ...
      <div className="bg-white rounded-xl border border-gl-border shadow-soft p-4 mb-6">
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="px-3 py-2 border border-gl-border rounded-lg text-sm  bg-white focus:outline-none focus:border-gl-blue">
            <option value="all">All Status</option>
            {Object.values(LeadMagnetStatus).map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
          <select value={filterFormat} onChange={(e) => setFilterFormat(e.target.value)} className="px-3 py-2 border border-gl-border rounded-lg text-sm  bg-white focus:outline-none focus:border-gl-blue">
            <option value="all">All Formats</option>
            {Object.values(MagnetFormat).map((f) => <option key={f} value={f}>{f}</option>)}
          </select>
          {niches.length > 0 && (
            <select value={filterNiche} onChange={(e) => setFilterNiche(e.target.value)} className="px-3 py-2 border border-gl-border rounded-lg text-sm  bg-white focus:outline-none focus:border-gl-blue">
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-gl-border shadow-soft px-8 py-16 text-center">
          <FileText className="w-10 h-10 text-gl-accent mx-auto mb-4" />
          <p className="text-sm text-gl-muted mb-1">
            {magnets.length === 0 ? 'Your resource vault is empty.' : 'No magnets match your filters.'}
          </p>
          {magnets.length === 0 && (
            <button onClick={() => onNavigate(AppPage.GENERATE)} className="text-sm text-gl-blue hover:underline mt-2">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((m) => (
            <div key={m.id} className="bg-white rounded-xl border border-gl-border shadow-soft hover:shadow-elegant transition-elegant group">
              <div className="p-5">
                ...
                <h3 className=" font-medium text-gl-ink text-sm leading-snug mb-2 line-clamp-2">
                  {m.title}
                </h3>
                <div className="flex flex-wrap gap-1.5 mb-3">
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-gl-bg text-gl-muted">{m.niche}</span>
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-gl-bg text-gl-muted">{m.format}</span>
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-gl-bg text-gl-muted">{m.persona}</span>
                </div>
